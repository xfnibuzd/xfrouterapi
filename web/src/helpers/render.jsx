...
import { ProviderIcon, getProviderIconByName } from '../components/ProviderIcon';
...
export const getModelCategories = (() => {
  ...
  return (t) => {
    ...
    categoriesCache = {
      ...
      anthropic: {
        label: 'Anthropic',
        icon: <ProviderIcon name='Claude' />,
        filter: (model) => model.model_name.toLowerCase().includes('claude'),
      },
      gemini: {
        label: 'Gemini',
        icon: <ProviderIcon name='Gemini' />,
        filter: (model) =>
      ...
      moonshot: {
        label: 'Moonshot',
        icon: <ProviderIcon name='Moonshot' />,
        filter: (model) =>
...
export function getChannelIcon(channelType) {
  ...
  const channelProviderMap = {
    1: 'OpenAI', 3: 'OpenAI', 57: 'OpenAI',
    2: 'Midjourney', 5: 'Midjourney',
    36: 'Suno',
    4: 'Ollama',
    14: 'Claude', 33: 'Claude',
    41: 'Gemini',
    ...
    51: 'Jimeng',
    56: 'Replicate',
    8: 'FastGPT', 22: 'FastGPT',
  };
  const provider = channelProviderMap[channelType];
  return provider ? <ProviderIcon name={provider} size={iconSize} /> : null;
}

/**
 * 根据图标名称动态获取供应商图标组件
 * 支持 "OpenAI"、"Claude.Color" 等格式（.Color/.Avatar 后缀会被忽略）
...
export function getLobeHubIcon(iconName, size = 14) {
  return getProviderIconByName(iconName, size);
}
